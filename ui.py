...
class ChatUI:
    ...
    def init_font(self):
        if not self.font:
            ...
            try:
                # Clean modern fonts
                self.font = pg.font.SysFont("helvetica, arial, sans-serif", self.font_size)
            except:
    ...
    def add_message(self, text: str, sender: str = "system"):
        ...
        if sender == "eve": color = Colors.EVE
        elif sender == "user": color = Colors.USER
        elif sender == "system": color = Colors.SYSTEM
        elif sender == "error": color = Colors.ERROR
    ...
    def handle_event(self, event: pg.event.Event):
        ...
        if event.type == pg.KEYDOWN:
            ...
            elif event.key == pg.K_v and (pg.key.get_mods() & pg.KMOD_CTRL or pg.key.get_mods() & pg.KMOD_META):
            ...
            elif event.key == pg.K_t and not self.active and not (pg.key.get_mods() & pg.KMOD_CTRL):
                self.active = True
                return None
            elif event.key == pg.K_ESCAPE:
                self.active = False
            else:
                if self.active and event.unicode and event.unicode.isprintable() and event.key != pg.K_RETURN:
    ...
    def draw(self, rect: pg.Rect):
        ...
        # Right border
        pg.draw.line(self.surface, Colors.BORDER, (chat_x + chat_w - 1, chat_y), (chat_x + chat_w - 1, chat_y + chat_h), 1)
        ...
        for line in self.input_lines:
            ...
            if not wrapped: wrapped = [""] # Empty line
        ...
        pg.draw.line(self.surface, Colors.BORDER, (chat_x, input_y), (chat_x + chat_w, input_y), 1)
        ...
        for msg in self.messages:
            ...
            wrapped_messages.append({"lines": lines, "color": msg["color"], "height": h})
            total_msg_height += h + (self.line_height // 2) # Add spacing between messages
